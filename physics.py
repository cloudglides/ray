...
def integrate_rk4(
    ...
    k1_vel = body.vel
    k1_acc = calculate_total_acceleration(body.pos, bodies, self_id, min_dist)
    k2_pos = body.pos + k1_vel * (0.5 * dt)
    k2_vel = body.vel + k1_acc * (0.5 * dt)
    k2_acc = calculate_total_acceleration(k2_pos, bodies, self_id, min_dist)
    k3_pos = body.pos + k2_vel * (0.5 * dt)
    k3_vel = body.vel + k2_acc * (0.5 * dt)
    k3_acc = calculate_total_acceleration(k3_pos, bodies, self_id, min_dist)
    k4_pos = body.pos + k3_vel * dt
    k4_vel = body.vel + k3_acc * dt
    k4_acc = calculate_total_acceleration(k4_pos, bodies, self_id, min_dist)
