...
def gravity_acceleration(
    pos: Vector2, other_pos: Vector2, other_mass: float, min_dist: float = 1.0
) -> Vector2:
    r = other_pos - pos
...
def calculate_total_acceleration(
    pos, bodies, self_id, spatial_grid=None, cell_size=None, min_dist=1.0
):
    acc = Vector2()
...
def integrate_rk4(
    ...
    k1_vel = body.vel
    k1_acc = calculate_total_acceleration(
        body.pos, bodies, self_id, spatial_grid, cell_size, min_dist
    )
    k2_pos = body.pos + k1_vel * (0.5 * dt)
    k2_vel = body.vel + k1_acc * (0.5 * dt)
    k2_acc = calculate_total_acceleration(
        k2_pos, bodies, self_id, spatial_grid, cell_size, min_dist
    )
    k3_pos = body.pos + k2_vel * (0.5 * dt)
    k3_vel = body.vel + k2_acc * (0.5 * dt)
    k3_acc = calculate_total_acceleration(
        k3_pos, bodies, self_id, spatial_grid, cell_size, min_dist
    )
    k4_pos = body.pos + k3_vel * dt
    k4_vel = body.vel + k3_acc * dt
    k4_acc = calculate_total_acceleration(
        k4_pos, bodies, self_id, spatial_grid, cell_size, min_dist
    )
